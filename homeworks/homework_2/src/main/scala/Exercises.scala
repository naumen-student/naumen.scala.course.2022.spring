object Exercises {
    ...
    def primeFactor(number: Int, list: List[Int] = List()): Seq[Int] = {
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] =
        ballsArray
          .mapValues(ballData => calculateBallWeight(ballData._1, ballData._2))
          .toSeq
          .sortBy(ball => ball._2)
          .map(ball => ball._1)

    def calculateBallWeight(radius: Int, density: Double): Double =
