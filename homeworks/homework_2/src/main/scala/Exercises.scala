object Exercises {
    ...
    def primeFactor(number: Int, primeNumbers:Seq[Int] = Seq.empty[Int], i: Int = 2) : Seq[Int] = {
        var num = number
        if (num <= 1 ) return primeNumbers
        if (num % i == 0) {
            while (num % i == 0) {
                num /= i
            }
            return primeFactor(num, primeNumbers :+ i, i + 1)
        }
        primeFactor(num, primeNumbers, i + 1)
    ...
    def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, func: (Vector2D, Vector2D) => Double,
                  rightVec0: Vector2D, rightVec1: Vector2D) = func(leftVec0, leftVec1) + func(rightVec0, rightVec1)
    ...
    */
    ...
    Map(
        ...
        "Lithium" -> (12,
          0.534), "Magnesium" -> (10, 1.738), "Copper" -> (3, 8.96), "Sodium" -> (5, 0.971),
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ...
        ballsArray.toSeq.map(ball => (ball._1, ballsWeight(ball._2._2, ball._2._1)))
          .sortBy(_._2)
          .map(_._1)

    }

    def ballsWeight(density:Double, radius:Int): Double = {
        density * 4 / 3 * java.lang.Math.PI * Math.pow(radius, 3)
    }
