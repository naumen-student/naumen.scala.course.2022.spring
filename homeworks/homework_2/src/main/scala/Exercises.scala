object Exercises {
    ...
    def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, funcForExecute: (Vector2D, Vector2D) => Double, rightVec0: Vector2D, rightVec1: Vector2D): Double = {
        funcForExecute(leftVec0, leftVec1) + funcForExecute(rightVec0, rightVec1)
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        def countVolume(radius: Int): Double = {
            4.0 / 3 * Math.PI * Math.pow(radius, 3)
        }

        def countMass(volume: Double, density: Double): Double = {
            volume * density
        }

        def restructData(data: (String, (Int, Double))): (Double, String) = {
            Tuple2(countMass(countVolume(data._2._1), data._2._2), data._1)
        }

        var restructedData = List[(Double, String)]()
        for (data <- ballsArray) {
            restructedData = restructedData :+ restructData(data)
        }
        restructedData = restructedData.sorted
        var res = List[String]()
        for (now <- restructedData) {
            res = res :+ now._2
        }
        res
    }
