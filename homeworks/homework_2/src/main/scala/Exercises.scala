...
object Exercises {
  ...
  def primeFactor(number: Int): Seq[Int] = {
    ...
    @tailrec
    def findDividers(dividers: Set[Int], num: Int): Set[Int] =
      if (dividers.isEmpty && num % 2 == 0) findDividers(dividers + 2, num / 2)
      else if (num != 1) {
        val divider = ((if (dividers.nonEmpty) dividers.max else 3) to num).filter(num % _ == 0).min
        findDividers(dividers + divider, num / divider)
      }
      else dividers

    findDividers(Set.empty[Int], number).toSeq
  }
  ...
  */
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] =
    ballsArray.toSeq.sortBy { case (_, (r, ro)) =>
      4 / 3 * java.lang.Math.PI * java.lang.Math.pow(r, 3) * ro
    }.map(_._1)
