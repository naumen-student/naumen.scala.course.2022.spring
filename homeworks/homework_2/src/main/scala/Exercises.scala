object Exercises {
    ...
    def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, func: (Vector2D, Vector2D) => Double, rightVec0: Vector2D, rightVec1: Vector2D) =
        ratio(leftVec0, leftVec1) + ratio(rightVec0, rightVec1)
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.map(ball => (ball._1, (4 / 3 * java.lang.Math.PI * Math.pow(ball._2._1,3)) * ball._2._2))
          .toSeq
          .sortBy(_._2)
          .map(_._1)
    }
