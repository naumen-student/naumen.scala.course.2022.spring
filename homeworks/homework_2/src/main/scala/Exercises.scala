...
import scala.math.{Pi, pow, random}

object Exerсises extends App {
    ...
    на 3 или на 5.*/
    /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = (iFrom to iTo).toList.filter(i => i % 3 == 0 || i % 5 == 0).sum
    ...
    def primeFactor(number: Int): Seq[Int] = {
        @tailrec
        def iter(x: Int, divider: Int, result: Seq[Int]): Seq[Int] = {
            if (x <= 1) result.distinct
            else if (x % divider == 0) {
                iter(x / divider, divider, result ++ Seq(divider))
            }
            else iter(x, divider + 1, result)
        }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.map {
            case (x, (y, z)) => (x, z * 4 / 3 * Pi * pow(y, 3))
        }.toSeq.sortWith(_._2 < _._2).map {
            case (s, _) => s
        }
    }
