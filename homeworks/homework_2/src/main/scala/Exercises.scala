...
object Exercises {
    ...
    на 3 или на 5.*/
    /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = List.range(iFrom, iTo + 1).filter(x => (x % 3) * (x % 5) == 0).sum
    ...
    /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
    def primeFactor(number: Int, m: Int = 2): Seq[Int] =
        if (number < m)
            Seq()
        else
            number % m match {
                case 0 => Seq(m) ++ primeFactor(divideWhileCan(number, m), m + 1)
                case _ => primeFactor(number, m + 1)
            }

    @tailrec
    private final def divideWhileCan(number: Int, divider: Int) : Int =
        if (divider == 1 || number % divider != 0)
            number
        else
            divideWhileCan(number / divider, divider)
    ...
    def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, transform: (Vector2D, Vector2D) => Double, rightVec0: Vector2D, rightVec1: Vector2D) : Double =
        transform(leftVec0, leftVec1) + transform(rightVec0, rightVec1)
    ...
    */
    ...
    private final def calculateBallVolume(radius: Int) : Double = 4.0 / 3.0 * Math.PI * radius * radius * radius
