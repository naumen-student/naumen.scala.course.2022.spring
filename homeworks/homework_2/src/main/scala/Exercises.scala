...
object Exercises {
  ...
  def primeFactor(number: Int): Seq[Int] = {
    var num = number.abs
    var i = 2;
    var primeFacs = new ListBuffer[Int]()

    while (i * i <= num) {//все делители находятся до корня числа
      while (num % i == 0) {
        num /= i
        primeFacs += i
      }
  ...
  def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, sumFunc: (Vector2D, Vector2D) => Double,
  ...
  */
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
    //m=p*4/3*pi*r^3 (4/3*pi можно опустить,т.к. одинакова для всех вычислений)
    val a = ballsArray.toSeq.sortBy(ball => scala.math.pow(ball._2._1, 3) * ball._2._2)
    a.map(ball => ball._1)
  }
