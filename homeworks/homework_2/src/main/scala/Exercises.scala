object Exercises {
  ...
  def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = {
    (iFrom to iTo).filter(p => p % 3 == 0 || p % 5 == 0).sum
  }
  ...
  */
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
    ballsArray.toSeq.sortBy(x => x._2._2 * java.lang.Math.PI * java.lang.Math.pow(x._2._1, 3) * 4 / 3).map(ball => ball._1)
  }
}