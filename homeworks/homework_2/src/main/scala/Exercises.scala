object Exercises {
    ...
    Можно не использовать число "Пи", так как необязательно находить массу шарика, достаточно того, что масса прямо пропорционально зависит от произведения density*radius^3
    */
    /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.toSeq.sortBy(x => x._2._1 * x._2._1 * x._2._1 * x._2._2).map(x => x._1)
    }
}