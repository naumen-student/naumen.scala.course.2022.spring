...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        (2 to number)
          .filter(x => number % x == 0 && isPrimeNumber(x))
    }

    private def isPrimeNumber(number: Int): Boolean = {
        !(2 to sqrt(number).toInt)
          .exists(i => number % i == 0)
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] =
        ballsArray
          .mapValues{ ball => calculateBallWeight(ball._1, ball._2) }
          .toSeq
          .sortBy{ _._2 }
          .map{ _._1 }

    private def calculateBallWeight(radius: Int, density: Double): Double =
        density * math.Pi * math.pow(radius, 3) * 4 / 3
}