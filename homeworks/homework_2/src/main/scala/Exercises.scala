...
object Exercises {
  ...
  def primeFactor(number: Int): Seq[Int] =
    (2 to number).filter(x => number % x == 0 && isPrime(x))

  private def isPrime(number: Int): Boolean =
    !(2 to sqrt(number).toInt).exists(i => number % i == 0)
  /*ЗАДАНИЕ III*/
  ...
  private def getWeight(radius: Int, density: Double) =
    density * 4 / 3 * Math.PI * pow(radius, 3)
}
