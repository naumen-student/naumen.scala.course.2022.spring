...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        (2 to number).filter(x => number % x == 0  && checkIsSimpleNumber(x))
    }

    def checkIsSimpleNumber(number: Int): Boolean = {
        (2 to sqrt(number).toInt).forall(x => number % x != 0)
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = ballsArray
      .toList
      .sortBy(x => x._2._2 * x._2._1 * x._2._1 * x._2._1  * 4/3 * math.Pi)
      .map(x => x._1)
}