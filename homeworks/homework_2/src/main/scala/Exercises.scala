...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        var mutableNumber = if (number > 0) number else -number
        var currentDenominator = 2
        val uniqueSimplePrimeDenominators :ListBuffer[Int] = ListBuffer()
        while (mutableNumber > 1) {
            if (mutableNumber % currentDenominator == 0) {
                uniqueSimplePrimeDenominators += currentDenominator
                while (mutableNumber % currentDenominator == 0) {
                    mutableNumber /= currentDenominator
                }
            }
            currentDenominator += 1
        }
        uniqueSimplePrimeDenominators.toSeq
    }
    ...
    def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, operation: (Vector2D, Vector2D) => Double, rightVec0: Vector2D,
                  rightVec1: Vector2D) = {
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.toSeq.sortBy(ball => PI * ball._2._1 * ball._2._1 * ball._2._1 * ball._2._2 * 4 / 3).unzip._1
    }
}
