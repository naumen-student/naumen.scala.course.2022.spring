...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        def divideEvenNumber(number: Int): Int = {
          if (number % 2 == 1) number
          else divideEvenNumber(number / 2)
        }
        
        def divideOddNumber(number: Int): Seq[Int] = {
            def loop(n: Int, firstDivider: Int = 3, acc: Seq[Int] = Seq[Int]()): Seq[Int]  = {
              if (n == 1) acc
              else {
                  var divider = firstDivider
                  while (n % divider != 0 && divider * divider <= n) 
                    divider += 2
                  if (n % divider == 0) {
                    loop(n / divider, divider, acc ++ Seq(divider))
                  }
                  else {
                    loop(1, acc=acc ++ Seq(n)) 
                  }                        
              }
            }
        ...
        def primeMultipliers(oddNumber: Int): Seq[Int] = {
            if (number != oddNumber) Seq(2) ++ divideOddNumber(oddNumber).distinct
            else divideOddNumber(oddNumber).distinct
        }
        
            
        primeMultipliers(divideEvenNumber(number))
    }
    ...
    def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, function: (Vector2D, Vector2D) => Double, rightVec0: Vector2D, rightVec1: Vector2D) = function(leftVec0, leftVec1) + function(rightVec0, rightVec1)
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        def sortByWeight(elem1: (String, (Int, Double)), 
                         elem2: (String, (Int, Double))): Boolean = {
            // val constPart = java.lang.Math.PI * 4/3
            elem1._2._1 * elem1._2._1 * elem1._2._2 < elem2._2._1 * elem2._2._1 * elem2._2._2 
        }
        
        ListMap(ballsArray.toSeq.sortWith((elem1, elem2) => 
                                     sortByWeight(elem1, elem2)
                                     ):_*).keys.toSeq
    }
