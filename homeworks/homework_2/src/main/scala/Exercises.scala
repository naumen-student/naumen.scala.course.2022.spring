...
object Exercises {
  ...
  def primeFactor(number: Int): Seq[Int] = {
    (2 to number)
      .filter(x => number % x == 0
        && (2 to sqrt(x).toInt).forall(el => x % el != 0))
  }
  ...
  */
  ...
  def calcBallWeight(radius: Int, density: Double): Double = (4 / 3) * math.Pi * math.pow(radius, 3) * density

  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] =
    ballsArray
      .mapValues({ case (r, d) => calcBallWeight(r, d) })
      .toSeq
