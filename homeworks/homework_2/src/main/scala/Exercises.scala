...
object Exercises {
    ...
    Range
      ...
      .foldLeft(Monoid[Long].empty)(_ + _)
    ...
    Range
      .inclusive(2, number / 2)
      .foldLeft(Nil: List[Int])((acc, i) =>
          i match {
              case i if number % i == 0 && !acc.exists(i % _ == 0) => i :: acc
              case _                                               => acc
          }
      )
      .reverse
    ...
    def sumByFunc[A, B](leftVec0: A, leftVec1: A, func: (A, A) => B, rightVec0: A, rightVec1: A)(implicit
                                                                                                 semigroup: Semigroup[B]
    ) = semigroup.combine(func(leftVec0, leftVec1), func(rightVec0, rightVec1))
    ...
    def sortByHeavyweight(
        ...
        ballsArray
          .mapValues(calcBallWeight)
          .toSeq
          .sortBy { case (name: String, weight: Double) => weight }
          .map { case (name: String, weight: Double) => name }

    private def calcBallVolume(radius: Int) = 4 / 3 * Pi * pow(radius, 3)
    private def calcBallWeight(ball: (Int, Double)) =
        ball match {
            case (radius: Int, density: Double) => calcBallVolume(radius) * density
        }
