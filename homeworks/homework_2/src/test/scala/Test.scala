...
object Test extends TestSuite{
    ...
    val tests = Tests{
        ...
        'test_sortByHeavyweight - {
            val balls1 = Map("Iron" -> (3,   7.874), "Aluminum" -> (3,   2.6889))
            assert(Exercises.sortByHeavyweight(balls1) == Seq("Aluminum", "Iron"))
            val balls2 = Map("Titanium" ->  (2,   10.50), "Plutonium" -> (3,   19.25))
            assert(Exercises.sortByHeavyweight(balls2) == Seq("Titanium", "Plutonium"))
        }
    }
