...
object ListOps {
  ...
  private def filterImpl[T](f: T => Boolean)(buffer: DataList[T])(l: DataList[T]): DataList[T] = l match {
    case DataList.NonEmptyList(head, tail) if f(head) => filterImpl(f)(NonEmptyList(head, buffer))(tail)
    case DataList.NonEmptyList(_, tail) => filterImpl(f)(buffer)(tail)
    case DataList.EmptyList => reverse(buffer)
  }

  private def reverse[T](list: DataList[T]): DataList[T] = {
    @tailrec
    def rev[T](resList: DataList[T], list: DataList[T]): DataList[T] = {
      list match {
        case DataList.EmptyList => resList
        case NonEmptyList(head, tail) => rev(NonEmptyList(head, resList), tail)
      }
    }
    rev(DataList.EmptyList, list)
  }
   ...
   */
  def collect[A, B](f: PartialFunction[A, B]): DataList[A] => DataList[B] = l =>
    map(f)(filter(f.isDefinedAt)(l))
