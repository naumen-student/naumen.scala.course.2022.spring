...
object ListOps {
  ...
  private def filterImpl[T](f: T => Boolean)(buffer: DataList[T])(l: DataList[T]): DataList[T] = l match {
    ...
    case EmptyList => listReverse(buffer, EmptyList)
  }

  def listReverse[T](l: DataList[T], buffer: DataList[T]): DataList[T] = l match{
    case NonEmptyList(head, tail) => listReverse(tail, NonEmptyList(head, buffer))
