...
object Exercises {
    ...
    private def boolToString(x: Boolean) = {
        if (x) {
            "правда"
        } else {
            "ложь"
    ...
    def sum3(x: Int, y: Int): Int = sumIntegers(SomeCollection(x, y))

    case class SomeCollection(numbers: Int*) extends Iterable[Int] {
        override def iterator: Iterator[Int] = numbers.toIterator
    }
}
