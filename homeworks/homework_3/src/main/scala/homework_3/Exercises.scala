...
object Exercises {
    ...
    def prettyBooleanFormatter1(x: Any): String = x match {
        case boolean: Boolean => boolToString(boolean)
        case _ => x.toString
    }

    def prettyBooleanFormatter2(x: Any): String = {
        if (x.isInstanceOf[Boolean])
            boolToString(x.asInstanceOf[Boolean])
        else
            x.toString
    }
    ...
    def boolToString(x: Boolean): String = if (x) "правда" else "ложь"
    ...
    def sum3(x: Int, y: Int): Int = sumIntegers(IterablePair(x, y))

    case class IterablePair(x: Int, y: Int) extends Iterable[Int] {
