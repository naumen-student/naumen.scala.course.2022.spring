...
object Exercises {
    ...
    def prettyBooleanFormatter1(x: Any): String = x match {
        case b: Boolean if b => "правда"
        case b: Boolean if !b => "ложь"
    ...
    def prettyBooleanFormatter3(x: Any): String =
        Option(x).collect {
            case x: Boolean if x => "правда"
            case x: Boolean if !x => "ложь"
    ...
    implicit class SafeMax[COLL, A](t: COLL)(implicit tr: COLL => Seq[A], ord: Ordering[A]) {
        def safeMax: Option[A] = {
            val coll = tr(t)
            if (coll.isEmpty) None else Some(coll.max)
        }
    }

    def max3(xs: Seq[Int]): Option[Int] = xs.safeMax
    ...
    class MyIterable[A](col: A*) extends Iterable[A] {
        override def iterator: Iterator[A] = col.iterator
    }
}
