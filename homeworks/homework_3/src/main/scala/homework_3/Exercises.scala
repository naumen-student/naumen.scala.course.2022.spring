...
object Exercises {
    ...
    def prettyBooleanFormatter1(x: Any): String = x match {
        case input: Boolean => if (input == true) "правда" else "ложь"
        case _ => x.toString 
    }
    ...
    def max2(xs: Seq[Int]): Seq[Int] = 
        if (xs.nonEmpty) Seq(xs.max) else Seq()
    ...
    def sum3(x: Int, y: Int): Int = sumIntegers(new IntegersCollection(x, y))

    class IntegersCollection(x: Int, y: Int) extends Iterable[Int] {
        override def iterator = Seq(x, y).iterator 
    }
